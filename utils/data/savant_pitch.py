...
def make_sql_load_pitch_data(
    ...
    """
    Makes the DuckDB SQL for the `load_pitch_data()` function below. Generally
    helpful in pitch-modeling context to use this as your CTE, and then build on
    top of it.
    ...
    SELECT
        ...
        p.balls,
        p.strikes,
        CONCAT(p.balls, '_', p.strikes) AS count_,
        CONCAT(p.season, '_', p.balls, '_', p.strikes) AS season_count,
        ...
        p.pitch_type,
        p.pitch_group,
        IF(p.pitch_group = 'FA', 1, 0) AS is_FA,
        IF(p.pitch_group = 'BB', 1, 0) AS is_BB,
        IF(p.pitch_group = 'OS', 1, 0) AS is_OS,
        ...
        p.plate_z,
        (p.plate_z - (p.sz_top + p.sz_bot) / 2) / ((p.sz_top - p.sz_bot) / 2) AS plate_z_sc,
        p.sz_top,
        p.sz_bot,
