...
N_SIMS = 10_000
HOME_LEADS = torch.arange(-15, 16)
PROB_RS_COLS = ["p_" + str(item) for item in RUNS_ARRAY]

LOGGER = get_logger()


def query_re24_values() -> pd.DataFrame:
    """
...
def simulate_regular_home_win_probs(
    p_runs: dict[tuple[int, str], td.Categorical], hm_win_prob_t10: float
) -> pd.DataFrame:
    """
    simulate home win probabilities for all regular inning game states via simulation.
    ...
    for game_state, inning, half_inning in tqdm(itertools.product(GAME_STATES, range(1, 11), (0, 1))):
        ...
        # simulate wins. TODO: recursive extras computation
...
def simulate_start_of_extras_home_win_probs(p_runs: dict[tuple[int, str], td.Categorical]) -> float:
    """
    simulate home win probability at start of 10th inning (tied after 9).
...
def simulate_win_probs() -> None:
    ...
    LOGGER.info("RE24 loaded")

    # make torch distributions
    p_runs = make_run_probability_dists(re24)
    LOGGER.info("Run scoring distributions initialized")

    # run simulations
    hm_win_prob_t10 = simulate_start_of_extras_home_win_probs(p_runs)
    win_probs = simulate_regular_home_win_probs(p_runs, hm_win_prob_t10)
